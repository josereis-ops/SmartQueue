-- MS-10b: RPC supervisao — loja=ponto, skill=equipa
...
BEGIN
  IF NOT public.has_permissao('supervisao.dashboard') THEN
    RETURN jsonb_build_object(
      ...
      'mensagem', 'Sem permissÃ£o para aceder Ã  Sala de Controlo.'
    );
  END IF;
  ...
  -- AgregaÃ§Ã£o fila + listas drill-down
  FOR v_caso IN
    SELECT
      c.*,
  ...
  LOOP
    ...
    v_caso_json := jsonb_build_object(
      ...
      'estado', CASE v_caso.status
        ...
        WHEN 'concluido' THEN 'ConcluÃ­do'
  ...
  v_fila := jsonb_build_object(
    ...
    'atrasadosTrabalho', (
      SELECT COUNT(*)::int FROM jsonb_array_elements(v_lista_atras) el
      WHERE el->>'estado' NOT IN ('Livre', 'Por tratar', 'ConcluÃ­do', 'Cancelado')
    ),
  ...
  LOOP
    ...
    v_presenca_lbl := CASE v_u.presenca
      WHEN 'disponivel' THEN 'DisponÃ­vel'
...
-- ---------------------------------------------------------------------------
-- get_perfil_utilizador — expor ponto de atendimento + skills (GAS col C / col M)
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'utilizador', jsonb_build_object(
      ...
      'skills', COALESCE(v_skills_csv, v_user.equipa_nome),
      'ponto_atendimento_id', v_user.ponto_atendimento_id,
      'ponto_atendimento', v_user.ponto_nome,

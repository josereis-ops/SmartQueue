-- MS-21: Cards Sala de Controlo só colaborador/supervisor + atribuição colaborador→supervisor

ALTER TABLE public.utilizadores
  ADD COLUMN IF NOT EXISTS supervisor_id UUID REFERENCES public.utilizadores (id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_utilizadores_supervisor ON public.utilizadores (supervisor_id);

-- Colaboradores sem supervisor: fallback GAS equipaOp (mesma skill primária que o supervisor)
COMMENT ON COLUMN public.utilizadores.supervisor_id IS
  'Responsável de equipa (perfil supervisor). Só para colaboradores operacionais.';
...
IMMUTABLE
AS $$
  SELECT COALESCE(NULLIF(trim(p_slug), ''), p_role::text) IN ('colaborador', 'supervisor');
$$;

-- Patch fusão OAuth: referências supervisor_id
...
BEGIN
  ...
  INSERT INTO public.utilizadores (
    id, area_id, equipa_id, ponto_atendimento_id, supervisor_id, email, nome, role, perfil_id, presenca, ultimo_ping, criado_em
  ) VALUES (
...
-- obter_dados_gestao_equipa: expor supervisor_id
...
-- guardar_utilizador: p_supervisor_id para colaboradores
...
BEGIN
  ...
  IF v_perfil_slug = 'colaborador' AND p_supervisor_id IS NOT NULL THEN
    IF NOT EXISTS (
      ...
      WHERE sup.id = p_supervisor_id
        ...
        AND COALESCE(ps.slug, sup.role::text) = 'supervisor'
    ) THEN
      RETURN jsonb_build_object('sucesso', false, 'mensagem', 'Supervisor de equipa invalido.');
    END IF;
  ...
  INSERT INTO public.utilizadores (
    id, area_id, equipa_id, ponto_atendimento_id, supervisor_id, email, nome, role, perfil_id, presenca
  ) VALUES (
...
-- Patch obter_dados_supervisao: cards só operacionais + isSuper só supervisor
  ...
  FROM (
    SELECT
      jsonb_build_object(
        ...
        'isSuper', (
          COALESCE(p.slug = 'supervisor', false)
          OR (p.id IS NULL AND u.role = 'supervisor')
        ),
    ...
    WHERE u.area_id = v_area_id
      AND public._utilizador_exibir_card_sala(COALESCE(p.slug, u.role::text), u.role)
  ) sub;

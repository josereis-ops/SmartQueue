-- MS-17d: skills GAS — expor Producao/Consumo (nome), não PROD/CONS (codigo)
...
DECLARE
  ...
  v_n       INT;
BEGIN
  UPDATE public.equipas e
  SET nome = 'Producao', codigo = 'Producao'
  WHERE e.area_id = v_area_id
    AND (
      e.id = 'b0000000-0000-4000-8000-000000000201'::uuid
      OR e.codigo IN ('PROD', 'Producao', 'PRODUCAO', 'producao')
      OR lower(trim(e.nome)) IN ('producao', 'produção', 'producao')
    )
    AND (e.nome IS DISTINCT FROM 'Producao' OR e.codigo IS DISTINCT FROM 'Producao');

  GET DIAGNOSTICS v_n = ROW_COUNT;
  RAISE NOTICE 'MS-17d: % equipa(s) Producao normalizada(s).', v_n;

  UPDATE public.equipas e
  SET nome = 'Consumo', codigo = 'Consumo'
  WHERE e.area_id = v_area_id
    AND (
      e.id = 'b0000000-0000-4000-8000-000000000202'::uuid
      OR e.codigo IN ('CONS', 'Consumo', 'CONSUMO', 'consumo')
      OR lower(trim(e.nome)) IN ('consumo')
    )
    AND (e.nome IS DISTINCT FROM 'Consumo' OR e.codigo IS DISTINCT FROM 'Consumo');

  GET DIAGNOSTICS v_n = ROW_COUNT;
  RAISE NOTICE 'MS-17d: % equipa(s) Consumo normalizada(s).', v_n;
END;
$$;

-- Grelha supervisão: coluna skill = nome da equipa (GAS col O), não codigo abreviado
...
BEGIN
  ...
  LOOP
    ...
    v_caso_json := jsonb_build_object(
      ...
      'caso_id', v_caso.id,
      'skill', COALESCE(v_caso.equipa_nome, '-'),
